...
module.exports = cds.service.impl(function () {
    ...
    const { Sales, SalesItems, SalesPayStatus, SalesReturnStatus, Items,MockCustomers,RetailLedger,Departments,PassbookEntryTypes,Inventory } = this.entities;
...
this.on('generateInvoice', async (req) => {
    ...
    this.on('payForPurchase',async(req)=>{
        ...
        for(const item of saleRecord.items){
            updateLedger(item,req);
            req.info("Ledger Updated")
            updateInventory(item,req);
    ...
    async function updateLedger(item,req){
        const deptRecord = await SELECT.one.from(Departments).where({ dept: 'SALES' });
        if (!deptRecord) return req.error(400, 'Invalid Department');
        
        const debitRecord = await SELECT.one.from(PassbookEntryTypes).where({ entryType: 'CREDIT' })
        if (!debitRecord) return req.error(400, 'CREDIT not found')
            ...
            .orderBy('createdAt desc');
                ...
                let newBalance = previousBalance + item.totalPayableAmount;
    ...
    async function updateInventory(item,req){
